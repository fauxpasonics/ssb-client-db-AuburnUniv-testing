...
--GO
--
--/****** Object:  View [etl].[vwTI_PatronMDM]    Script Date: 5/31/2017 2:39:42 PM ******/
--SET ANSI_NULLS ON
--GO
--
--SET QUOTED_IDENTIFIER ON
...
(
	...
	, patron.Title
	, patron.NAME AS FullName
	, patron.SUFFIX AS Suffix
	...
	--, CAST(ISNULL(pa.ADDR1,'') + ' ' + ISNULL(pa.ADDR2,'') + ' ' + ISNULL(pa.ADDR3,'') + ' ' + ISNULL(pa.ADDR4,'') AS VARCHAR(500)) PrimaryAddressStreet
    -- , NULLIF(CAST(dbo.city(zip.CSZ) AS VARCHAR(200)) = '') PrimaryAddressCity, CAST(dbo.State(zip.CSZ) AS VARCHAR(200)) PrimaryAddressState, CAST(dbo.Zip(zip.CSZ) AS VARCHAR(200)) PrimaryAddressZipCode, CAST(pa.COUNTRY AS VARCHAR(200)) PrimaryAddressCountry
	...
	, NULL AS DonorID
	, patron.STATUS COLLATE SQL_Latin1_General_CP1_CS_AS PatronStatusCode 
	, pdStatus.Name PatronStatus
	, Patron.vip AS VIP
	, ISNULL(cust.C_PRIORITY, 0) PriorityPtsTix	
	, NULL AS Donor_Level
	, fdon.DONOR_TYPE AS Donor_Type
...
--	, pt.PHONE_TYPE
	...
	LEFT OUTER JOIN ( 
	--This is looking at the customers preferred address type in TK_CUSTOMER to pull that for Primary
			SELECT a.*
			FROM dbo.PD_ADDRESS (NOLOCK) a
			INNER JOIN dbo.TK_CUSTOMER t ON a.PATRON = t.CUSTOMER AND a.ADTYPE COLLATE SQL_Latin1_General_CP1_CS_AS = ISNULL(t.M_ADTYPE,'M') COLLATE SQL_Latin1_General_CP1_CS_AS
		
			--Need to add logic in here to account for t.M_ADTYPE null instances. This is causing the primary address to be ' '
	...
	LEFT OUTER JOIN (
		...
		FROM (
			...
			, ROW_NUMBER() OVER (PARTITION BY Patron ORDER BY VMC) RowRank
			FROM dbo.PD_PATRON_PHONE_TYPE (NOLOCK)
			WHERE PHONE_TYPE NOT IN ('E', 'SE') AND PHONE LIKE '%@%'
		) op
		WHERE op.RowRank = 1
...
WHERE paa5.PriorityRank = 4
	...
	LEFT OUTER JOIN dbo.SYS_ZIP (NOLOCK) zip ON CASE WHEN CHARINDEX('-', pa.SYS_ZIP) = 0 THEN pa.SYS_ZIP ELSE LEFT(pa.SYS_ZIP, CHARINDEX('-', pa.SYS_ZIP) - 1) END = zip.SYS_ZIP COLLATE SQL_Latin1_General_CP1_CS_AS
		...
		FROM (
			...
			SELECT DONOR AS Patron, Export_DATETIME --this only applies to clients with tFund
			FROM dbo.fd_donor (NOLOCK)
		) a
		GROUP BY Patron
