...
Created by AMeitin 2017/0512 - data sent to TI server dbo.CI_Adobe_Customer where FanOne picks up for integation in Adobe Campaign

CRM ID pieces will be added in once CRM Integration is complete
...
FROM dbo.vwDimCustomer_ModAcctID a
	LEFT JOIN (
					...
					FROM [AuburnUniv_Reporting].[ProdCopy].[Account] acct (NOLOCK) 
