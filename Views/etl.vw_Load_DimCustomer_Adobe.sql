...
DCH on 2018-02-18	-	added TRY_CAST to AccountId because some values overran the size limits of data type INT
...
*/
		...
		SELECT 
			...
			, CAST(CASE WHEN [FirstName] = ' ' THEN [LastName]
				   WHEN [LastName] = ' ' THEN [FirstName] 
				   ELSE NULL END AS NVARCHAR(50)) [CompanyName] 
			...
			, CASE WHEN [LastName] = ' ' THEN NULL 
				   WHEN [FirstName] = ' ' THEN NULL
				   ELSE [LastName] END AS [LastName]
			, NULL AS [MiddleName]
			, CASE WHEN [FirstName] = ' ' THEN NULL 
			...
			, CASE WHEN CAST([State] AS NVARCHAR(200)) = '' THEN NULL
				ELSE CAST([State] AS NVARCHAR(200)) END AS [AddressPrimaryState] 
			...
			, CASE WHEN CAST([CellPhone] AS NVARCHAR(25)) = ' ' THEN NULL ELSE CAST([CellPhone] AS NVARCHAR(25)) END AS [PhoneCell]
			, CASE WHEN CAST([BusinessPhone] AS NVARCHAR(25)) = ' ' THEN NULL ELSE CAST([BusinessPhone] AS NVARCHAR(25)) END AS [PhoneBusiness]
			, CASE WHEN CAST([Fax] AS NVARCHAR(25)) = ' ' THEN NULL ELSE CAST([Fax] AS NVARCHAR(25)) END AS [PhoneFax]
			...
			/*Extended Attributes*/
			, CAST(EmailOptOut_Preferences AS NVARCHAR(100)) AS[ExtAttribute1] --nvarchar(100)
			, CAST(Linked AS NVARCHAR(100)) AS[ExtAttribute2] 
			...
			, TRY_CAST(AccountId as int) [AccountId]
			, CASE WHEN [FirstName] = '' AND [LastName] IS NOT NULL THEN 1
				   WHEN [LastName] = '' AND [FirstName] IS NOT NULL THEN 1
				   ELSE 0 END AS IsBusiness
