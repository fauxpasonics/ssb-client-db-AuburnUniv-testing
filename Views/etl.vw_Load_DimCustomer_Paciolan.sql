...
*/
		...
		SELECT 
			...
			, CAST(CASE WHEN FullName like '% Inc' THEN FullName 
					   WHEN FullName like '% Inc.' THEN FullName
					   WHEN FullName like '% Incorporated %' THEN FullName
					   WHEN FullName like '% Co' THEN FullName
					   WHEN FullName like '%Trust' THEN FullName
					   WHEN FullName like '%Foundation' THEN FullName
					   WHEN FullName like '%Associates%' THEN FullName
					   WHEN FullName like '%Association%' THEN FullName
					   WHEN FullName like '%LLC' THEN FullName
					   WHEN FullName like '%L.L.C.' THEN FullName
					   WHEN FullName like '% Ltd' THEN FullName
					   WHEN FullName like '% Ltd.' THEN FullName
					   WHEN FullName like '% Limited' THEN FullName
					   WHEN FullName like '%Corp.' THEN FullName
					   WHEN FullName like '%Corp' THEN FullName
					   WHEN FullName like '%Corporation%' THEN FullName
					   WHEN FullName like '%Company' THEN FullName
					   WHEN FullName like '% P.C.' THEN FullName
					   WHEN FullName like '% PC' THEN FullName
					   WHEN FullName like '%Society%' THEN FullName
					   WHEN FullName like '%Association%' THEN FullName
					   WHEN FullName like '%Foundation%' THEN FullName
					   WHEN FullName LIKE '%,%' THEN NULL  -- keep as last WHEN before Else
					   ELSE FullName
					   END AS NVARCHAR(50)) CompanyName
	   , NULL AS [SalutationName]
			...
			, LEFT(FullName,200) + ' ' + ISNULL(p.Suffix,'') [FullName]
			...
			/*AddressPrimary*/
			, COALESCE(CASE WHEN PrimaryAddressStreet = '' THEN NULL ELSE PrimaryAddressStreet END, Address2Street, Address3Street, Address4Street, Address5Street) AS [AddressPrimaryStreet]
			, NULL AS [AddressPrimarySuite]
			, COALESCE(CASE WHEN PrimaryAddressCity = '' THEN NULL ELSE PrimaryAddressCity END, Address2City, Address3City, Address4City, Address5City) AS [AddressPrimaryCity] 
			, COALESCE(CASE WHEN PrimaryAddressState = '' THEN NULL ELSE PrimaryAddressState END, Address2state, Address3State, Address4State, Address5State) AS [AddressPrimaryState] 
			, COALESCE (CASE WHEN PrimaryAddressZipCode = '' THEN NULL ELSE PrimaryAddressZipCode END, Address2ZipCode, Address3ZipCode, Address4ZipCode, Address5ZipCode) AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, COALESCE(CASE WHEN PrimaryAddressCountry = '' THEN NULL ELSE PrimaryAddressCountry END, Address2Country, Address3Country,Address4Country,Address5Country) AS [AddressPrimaryCountry] 
			...
			, CAST(BusinessPhone AS NVARCHAR(25)) PhonePrimary
			...
			/*Extended Attributes*/
			, p.EXTERNAL_NUMBER AS[ExtAttribute1] --nvarchar(100)
			, p.PatronStatusCode COLLATE SQL_Latin1_General_CP1_CS_AS AS [ExtAttribute2] 
			, p.Donor_Type AS[ExtAttribute3] 
			...
			, 0 AS ISDeleted
			, NULL [AccountId]
			, CAST(CASE WHEN FullName like '% Inc' THEN 1 
			   WHEN FullName like '% Inc.' THEN 1
			   WHEN FullName like '% Incorporated %' THEN 1
			   WHEN FullName like '% Co' THEN 1
			   WHEN FullName like '%Trust' THEN 1
			   WHEN FullName like '%Foundation' THEN 1
			   WHEN FullName like '%Associates%' THEN 1
			   WHEN FullName like '%Association%' THEN 1
			   WHEN FullName like '%LLC' THEN 1
			   WHEN FullName like '%L.L.C.' THEN 1
			   WHEN FullName like '% Ltd' THEN 1
			   WHEN FullName like '% Ltd.' THEN 1
			   WHEN FullName like '% Limited' THEN 1
			   WHEN FullName like '%Corp.' THEN 1
			   WHEN FullName like '%Corp' THEN 1
			   WHEN FullName like '%Corporation%' THEN 1
			   WHEN FullName like '%Company' THEN 1
			   WHEN FullName like '% P.C.' THEN 1
			   WHEN FullName like '% PC' THEN 1
			   WHEN FullName like '%Society%' THEN 1
			   WHEN FullName like '%Association%' THEN 1
			   WHEN FullName like '%Foundation%' THEN 1
			   WHEN FullName LIKE '%,%' THEN 0 -- keep as last WHEN before Else
			   ELSE 1
		END AS BIT) IsBusiness
	   , 'Paciolan-' + [p].[Patron]  customer_MatchKey

--		select TOP 10 *
		FROM etl.vwTI_PatronMDM p
		WHERE 1=1
		AND UpdatedDate > DATEADD(DAY,-3,GETDATE())
