...
/***************
7/31/2017 - AMEITIN created - used Stanford as base with slight adjustments
7/23/2018 - AMEITIN adjusted delete logic
7/23/2018 - SSALES Approved new logic
...
--base set
		...
		'Auburn SFDC' AS [SourceDB]
		, 'Auburn PC_SFDC Account' AS [SourceSystem]
		, 1 AS [SourceSystemPriority]
		, CAST(a.[ID] AS NVARCHAR(50)) AS [SSID]

		/*Standard Attributes*/
		, SSB_CRMSYSTEM_Customer_Type__c AS [CustomerType]
		, Customer_Status__c AS [CustomerStatus]
		...
		, TRY_CAST(PersonBirthdate AS DATE) AS [Birthday]
		, NULL AS [Gender] 
		, 0 [MergedRecordFlag]
		, NULL [MergedIntoSSID]
		...
		/*AddressPrimary*/
			...
			WHERE 1=1
			--AND a.isdeleted = 0
			AND a.CopyLoadDate > DATEADD(DAY,-10,GETDATE())
